# Data: cohorts, sources, and the one trap to avoid

Four public cohorts, each from a different machine and population. That
...
leave-one-cohort-out fold. The label is the last digit of the filename
...
out-of-domain distractor, rather than as a TB cohort in its own right.
...
## Read this before you run leave-one-cohort-out
...
RSNA. If you build cohorts that way, the cohort label becomes an almost perfect
...
2. When a single-class cohort is the held-out fold, sensitivity or specificity
...
- If you use NIAID or RSNA, do it deliberately: mix RSNA normals and NIAID
  positives into balanced cohorts, or treat RSNA as a normals-only / OOD source
  and say so in the paper. Do not hold out an all-TB or all-normal cohort and
  report it as a clean generalization result.
- `scripts/build_manifest.py` prints a class-balance table and flags single-class
  cohorts. `xctb.data.splits.check_split` warns at split time. Heed both.

## After downloading

    python scripts/build_manifest.py --data-root data     # writes data/manifest.csv
    python scripts/cohort_stats.py --manifest data/manifest.csv   # domain-shift numbers
