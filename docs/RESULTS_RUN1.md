...
**Update (2026-08-21):** both issues above are resolved. The certificate layout
was fixed in `physics/figures.py` (the bars and provenance block now own fixed
regions and cannot drift into the action box) and all nine figures re-rendered.
The three real-image figures (`09`–`11`) were rendered from the raw NLM
Montgomery set after rebuilding the manifest locally. Two results figures were
added from the measured tables in this document — `e2_generalization_gap.png`
and `e3_accuracy_vs_severity.png`, generated by `scripts/make_results_figures.py`.
The reliability diagram, risk-coverage curves and per-clinic calibration heatmap
are deliberately not drawn: they need per-sample predictions
(`combined_predictions.csv`, Kaggle-only), and a curve reconstructed from a
summary scalar would be invented data.
